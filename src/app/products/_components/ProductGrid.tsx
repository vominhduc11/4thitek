...
const ProductGrid = ({ products }: ProductGridProps) => {
  ...
  const renderProductCard = (product: Product, index: number, isSecondRow: boolean = false) => (
    <motion.div
      key={product.id}
      layout
      ...
      style={isSecondRow ? { flex: "0 0 25%" } : undefined}
    >
      ...
      <motion.div
        className={`relative bg-gray-900/30 hover:bg-gray-800/50 transition-all duration-300 cursor-pointer group overflow-hidden min-h-[320px] xs:min-h-[360px] sm:min-h-[400px] md:min-h-[450px] lg:min-h-[500px] xl:min-h-[600px] flex flex-col rounded-lg lg:rounded-none ${
          !isSecondRow ? "border-t border-gray-700/50" : "border-t border-r border-gray-700/50"
        }`}
          ...
          <motion.div className="flex flex-col lg:flex-row relative" layout>
            <AnimatePresence>
              {products.slice(4).map((product, index) => 
                renderProductCard(product, index + 4, true)
              )}

              {/* Empty spaces to maintain layout consistency */}
              {Array.from(
                { length: 4 - products.slice(4).length },
                (_, index) => (
                  <div
                    key={`empty-${index}`}
                    className="relative"
