...
function WholesaleDiscountsPageRevamp() {
  ...
  const stats = useMemo(() => {
    const active = discountRules.filter(
      (item) => item.status === "active",
    ).length;
    const pending = discountRules.filter(
  ...
  if (
    discountRulesState.status === "loading" ||
    discountRulesState.status === "idle"
  ) {
    return (
      <PagePanel>
        <LoadingRows rows={5} />
