...
function OrderDetailPage() {
  ...
  const [adjustments, setAdjustments] = useState<
    BackendOrderAdjustmentResponse[]
  >([]);
  const [adjType, setAdjType] =
    useState<BackendOrderAdjustmentType>("CORRECTION");
    ...
    return (
      <PagePanel>
        <EmptyState
          ...
          message={t("Đơn {id} không tồn tại hoặc đã bị xóa.", {
            id: decodedId,
          })}
        />
        <div className="mt-4">
  ...
  const availableTransitions =
    order.allowedTransitions ?? getAllowedOrderStatuses(order.status);
  const canQuickComplete =
    order.status !== "completed" && availableTransitions.includes("completed");
    ...
    <PagePanel>
      ...
      {order.staleReviewRequired && (
        <div className="mt-4 flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800 dark:border-rose-800/40 dark:bg-rose-900/20 dark:text-rose-200">
          ...
          <span>
            {t(
              "Đơn này đã hết hạn xử lý nhưng có thanh toán ghi nhận — cần xem xét thủ công trước khi hủy.",
            )}
          </span>
        </div>
      )}
                ...
                try {
                  ...
                  notify(
                    error instanceof Error
                      ? error.message
                      : t("Không cập nhật được đơn hàng"),
                    {
                      title: t("Đơn hàng"),
                      variant: "error",
                    },
            ...
            <PrimaryButton
              ...
              onClick={async () => {
                const approved = await confirm({
                  ...
                  message: t("Đánh dấu đơn {id} đã hoàn tất?", {
                    id: order.orderCode,
                  }),
                  tone: "info",
            ...
            <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
              ...
              <div className="mt-3 grid gap-3">
                <label className="space-y-2">
                  ...
                  <input
                    aria-describedby={
                      paymentError ? "order-payment-amount-error" : undefined
                    }
                ...
                <PrimaryButton
                  onClick={async () => {
                    ...
                    try {
                      ...
                      notify(
                        t("Đã ghi nhận thanh toán cho đơn {id}", {
                          id: order.orderCode,
                        }),
                        {
                          title: t("Đơn hàng"),
                      ...
                      notify(
                        error instanceof Error
                          ? error.message
                          : t("Không ghi nhận được thanh toán"),
                        {
                          title: t("Đơn hàng"),
                          variant: "error",
                        },
      ...
      <div className="mt-6 rounded-3xl border border-slate-200/70 bg-white/80 p-5">
        <p className="text-sm font-semibold text-slate-900">
          {t("Điều chỉnh tài chính")}
        </p>
        <p className="mt-1 text-xs text-slate-500">
          {t("Ghi nhận điều chỉnh, bù trừ hoặc hoàn tiền cho đơn hàng.")}
        </p>
                ...
                {adjustments.map((adj) => (
                  <tr key={adj.id}>
                    ...
                    <td className="py-2 whitespace-nowrap">
                      {adj.createdAt ? formatDateTime(adj.createdAt) : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <label className="space-y-1">
            ...
            <select
              ...
              value={adjType}
              onChange={(e) =>
                setAdjType(e.target.value as BackendOrderAdjustmentType)
              }
            >
              ...
              <option value="REFUND_RECORD">
                {t("Ghi nhận hoàn tiền (REFUND_RECORD)")}
              </option>
            </select>
          </label>
          <label className="space-y-1">
        ...
        {adjError && (
          <FieldErrorMessage className={fieldErrorClass}>
            {adjError}
          </FieldErrorMessage>
        )}
        ...
        <div className="mt-3">
          <PrimaryButton
            ...
            onClick={async () => {
              ...
              try {
                const created = await createAdminOrderAdjustment(
                  accessToken,
                  Number(order.id),
                  {
                    type: adjType,
                ...
                setAdjReason("");
                setAdjRef("");
                notify(t("Đã thêm điều chỉnh tài chính"), {
                  title: t("Điều chỉnh"),
                  variant: "success",
                });
              } catch (err) {
