...
const STATUS_RESOLVE_OPTIONS: BackendUnmatchedPaymentStatus[] = [
  "MATCHED",
  "REFUNDED",
  "WRITTEN_OFF",
];
...
function UnmatchedPaymentsPageRevamp() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "ALL" | BackendUnmatchedPaymentStatus
  >("ALL");

  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [statusDraft, setStatusDraft] =
    useState<BackendUnmatchedPaymentStatus>("MATCHED");
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const updated = await resolveAdminUnmatchedPayment(
        accessToken,
        selectedItem.id,
        {
          status: statusDraft,
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <select
            ...
            onChange={(e) =>
              setStatusFilter(
                e.target.value as "ALL" | BackendUnmatchedPaymentStatus,
              )
            }
          >
            ...
            <div className={`${softCardClass} w-full xl:w-96 xl:shrink-0`}>
              ...
              <dl className="mt-4 space-y-2 text-sm">
                {[
                  ...
                  {
                    label: copy.orderHint,
                    value: selectedItem.orderCodeHint ?? "—",
                  },
                  {
                    label: copy.reason,
                    value: selectedItem.reason
                      ? copy.reasons[selectedItem.reason]
                      : "—",
                  },
                  {
                    label: copy.receivedAt,
                    value: selectedItem.receivedAt
                      ? formatDateTime(selectedItem.receivedAt)
                      : "—",
                  },
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        setSaveError(null);
                        setStatusDraft(
                          e.target.value as BackendUnmatchedPaymentStatus,
                        );
                      }}
                    >
