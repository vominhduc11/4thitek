...
const roleDescriptions = {
  vi: {
    'Admin hệ thống': 'Quyền quản trị toàn diện, bao gồm tài khoản nội bộ và cài đặt nhạy cảm.',
    'Quản lý sản phẩm': 'Phù hợp cho đội phụ trách danh mục, nội dung sản phẩm và trạng thái xuất bản.',
    'Marketing & Nội dung': 'Dành cho quản lý bài viết, truyền thông và nội dung hiển thị.',
    'CSKH & Bảo hành': 'Phục vụ theo dõi hỗ trợ, bảo hành và các đầu việc sau bán hàng.',
  },
  en: {
    'Admin hệ thống': 'Full admin access, including internal accounts and sensitive settings.',
    'Quản lý sản phẩm': 'Best for catalog management, product content, and publishing status.',
    'Marketing & Nội dung': 'For posts, campaigns, and storefront-facing content.',
    'CSKH & Bảo hành': 'For support, warranty, and after-sales operations.',
  },
} as const
...
const copyByLanguage = {
  ...
  en: {
    ...
    searchPlaceholder: 'Search by name, role, email, or id...',
    addUser: 'Invite user',
    total: 'Total users',
    active: 'Active',
    pending: 'Pending',
    createTitle: 'Invite a new account',
    createDescription:
    ...
    emailInvalid: 'Email is not in a valid format.',
    ...
    inviteSuccess: 'New user invite sent.',
    inviteFailed: 'Could not send the user invite.',
  },
} as const

const getInviteFormErrors = (
...
function UsersPageRevamp() {
  ...
  const filteredUsers = useMemo(
    () =>
      searchableUsers
        .filter(({ searchText }) => (!normalizedQuery ? true : searchText.includes(normalizedQuery)))
        .map(({ user }) => user),
  ...
  const selectedRoleDescription =
    roleDescriptions[language][form.role as keyof (typeof roleDescriptions)[typeof language]]
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      notify(saveError instanceof Error ? saveError.message : copy.inviteFailed, {
        title: copy.title,
        variant: 'error',
      })
    }
      ...
      {showInvite ? (
        <div className={`${formCardClass} mt-6`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <StatusBadge tone="warning">{t(userStatusLabel.pending)}</StatusBadge>
          </div>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="space-y-2 md:col-span-2">
              ...
              <input
                ...
                value={form.email}
              />
              {formErrors.email ? <FieldErrorMessage>{formErrors.email}</FieldErrorMessage> : null}
            </label>
            ...
            <label className="space-y-2">
              ...
              <input
                ...
                value={form.name}
              />
              {formErrors.name ? <FieldErrorMessage>{formErrors.name}</FieldErrorMessage> : null}
            </label>
            ...
            <label className="space-y-2">
              ...
              {selectedRoleDescription ? (
                <p className="text-sm text-[var(--muted)]">{selectedRoleDescription}</p>
              ) : null}
              {formErrors.role ? <FieldErrorMessage>{formErrors.role}</FieldErrorMessage> : null}
            </label>
          </div>
