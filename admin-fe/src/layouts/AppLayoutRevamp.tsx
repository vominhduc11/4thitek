...
import { ghostButtonClass, inputClass, tableMetaClass } from '../components/ui-kit'
...
const copyByLanguage = {
  vi: {
    ...
    welcomeText: 'Theo dõi vận hành, kho, dịch vụ sau bán và tài khoản nội bộ từ một nơi.',
  ...
  en: {
    ...
    welcome: 'Operations control panel',
    welcomeText: 'Monitor commerce, inventory, after-sales, and internal access from one place.',
    ...
    alertTemplates: {
      pendingOrders: '{count} orders are waiting for processing',
      lowStock: '{count} products are close to low-stock threshold',
      scheduledPosts: '{count} posts are still scheduled',
      dealerAttention: '{count} dealers need profile updates',
      pendingUsers: '{count} internal users are pending approval',
    },
    alertDescriptions: {
...
const writeAlertIds = (ids: Set<string>) => {
  ...
  try {
    window.localStorage.setItem(ALERT_READ_STORAGE_KEY, JSON.stringify(Array.from(ids)))
  } catch {
...
function AppLayoutRevamp() {
  ...
  const navItems = useMemo<NavItem[]>(() => {
    const items: NavItem[] = [
      { to: '/', label: copy.nav.dashboard, icon: LayoutDashboard, group: 'overview' },
      { to: '/reports', label: copy.nav.reports, icon: FileText, group: 'overview' },
      { to: '/products', label: copy.nav.products, icon: Package, group: 'commerce' },
      { to: '/orders', label: copy.nav.orders, icon: ShoppingCart, group: 'commerce' },
      { to: '/dealers', label: copy.nav.dealers, icon: UserCircle, group: 'commerce' },
      { to: '/discounts', label: copy.nav.discounts, icon: Percent, group: 'commerce' },
      { to: '/blogs', label: copy.nav.blogs, icon: BookOpenText, group: 'commerce' },
      { to: '/warranties', label: copy.nav.warranties, icon: ShieldCheck, group: 'service' },
  ...
  const groupedNav = useMemo(
    () =>
      (['overview', 'commerce', 'service', 'system'] as NavGroupId[]).map((groupId) => ({
        id: groupId,
  ...
  const alerts = useMemo<AlertItem[]>(() => {
    ...
    if (pendingOrders > 0) {
      items.push({
        id: 'pending-orders',
        title: interpolate(copy.alertTemplates.pendingOrders, { count: pendingOrders }),
    ...
    if (lowStockProducts > 0) {
      items.push({
        id: 'low-stock',
        title: interpolate(copy.alertTemplates.lowStock, { count: lowStockProducts }),
    ...
    if (scheduledPosts > 0) {
      items.push({
        id: 'scheduled-posts',
        title: interpolate(copy.alertTemplates.scheduledPosts, { count: scheduledPosts }),
    ...
    if (dealerAttention > 0) {
      items.push({
        id: 'dealer-attention',
        title: interpolate(copy.alertTemplates.dealerAttention, { count: dealerAttention }),
    ...
    if (hasRole('SUPER_ADMIN') && pendingUsers > 0) {
      items.push({
        id: 'pending-users',
        title: interpolate(copy.alertTemplates.pendingUsers, { count: pendingUsers }),
    ...
    >
      ...
      <div className="space-y-3 border-t border-white/10 pt-4 text-xs text-slate-400">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2">
          <p className="text-[11px] uppercase tracking-[0.28em] text-slate-500">{copy.account}</p>
                  ...
                  {isSearchOpen && globalQuery.trim() ? (
                    <div className="absolute left-0 right-0 z-30 mt-2 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-3 shadow-[0_18px_40px_rgba(15,23,42,0.12)]">
                      ...
                      {searchResults.length > 0 ? (
                        ...
                        <p className="px-2 py-4 text-sm text-[var(--muted)]">{copy.searchEmpty}</p>
                      )}
                    </div>
                  ) : null}
                </form>
                    ...
                    {isAlertsOpen ? (
                      ...
                      >
                        ...
                        {alerts.length > 0 ? (
                          ...
                          <p className="px-2 py-4 text-sm text-[var(--muted)]">{copy.alertsEmpty}</p>
                        )}
                      </div>
                    ) : null}
                  </div>
                    ...
                    {isAccountOpen ? (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() => {
                            logout()
                            notify(copy.logout, { title: copy.account, variant: 'info' })
                            navigate('/login')
                          }}
                          type="button"
                        >
