...
**Quy tắc nghiệp vụ cập nhật 2026-03-22:** Dealer **chỉ được khai báo yêu cầu ghi nhận thanh toán**, không tự xác nhận thanh toán cuối cùng. Bản ghi do dealer tạo phải đi qua bước duyệt của admin/kế toán trước khi được tính vào công nợ thực tế.

**Mô hình trạng thái đề xuất:**
- `PENDING_VERIFICATION` — dealer đã gửi yêu cầu + chứng từ, chờ kiểm tra
- `CONFIRMED` — admin/kế toán xác nhận hợp lệ, mới tính là đã thanh toán
- `REJECTED` — admin/kế toán từ chối, không làm thay đổi công nợ

**Validation khi dealer gửi yêu cầu:**
- `amount > 0` (`@DecimalMin(0.01)`)
- `amount ≤ outstandingAmount` tại thời điểm gửi yêu cầu
...
**Quy tắc hạch toán công nợ:**
- `declaredAmount`: số tiền dealer khai đã thanh toán
- `confirmedPaidAmount`: số tiền admin/kế toán xác nhận hợp lệ
- `outstandingAmount = max(0, total − confirmedPaidAmount)`

> `outstandingAmount` **không giảm ngay** khi dealer gửi yêu cầu. Chỉ sau khi admin/kế toán chuyển bản ghi sang `CONFIRMED`, công nợ mới được trừ thực tế.

#### Admin xác nhận payment — `POST /api/v1/admin/orders/{id}/payments`

Payload như dealer + không bị chặn bởi SePay restriction. Admin/kế toán có quyền:
- xác nhận yêu cầu thanh toán do dealer gửi
- từ chối yêu cầu không hợp lệ
- ghi payment trực tiếp trong các trường hợp ngoại lệ/override

> Admin không nên xóa cứng đơn ở trạng thái `CANCELLED`; ưu tiên soft-delete hoặc archive để giữ audit trail và phục vụ đối soát.
...
Tổng hợp: tổng nợ tồn (`Σ outstandingAmount`), số đơn còn nợ, danh sách có thể bấm để gửi yêu cầu ghi nhận thanh toán.

> Dealer có thể tạo yêu cầu ghi nhận thanh toán cho đơn còn nợ, nhưng công nợ chỉ giảm sau khi admin/kế toán xác nhận. Dealer không tự làm giảm `outstandingAmount`.
...
```
Dashboard → Shortcut "Công nợ" → Chọn đơn còn nợ → Gửi yêu cầu ghi nhận thanh toán (số tiền, kênh, chứng từ)
→ Hệ thống lưu yêu cầu ở trạng thái chờ xác minh
→ Admin/kế toán kiểm tra và xác nhận
→ confirmedPaidAmount tăng
→ outstandingAmount giảm
...
| Dealer gửi yêu cầu nhưng admin chưa duyệt | Chưa làm thay đổi công nợ thực, giữ `PENDING_VERIFICATION` |
| Admin từ chối yêu cầu | Không cộng vào `confirmedPaidAmount`, không giảm `outstandingAmount` |
...
| Ghi nhận thanh toán | Dealer gửi yêu cầu, chờ xác minh; bank transfer tự động qua SePay | Admin/kế toán xác nhận yêu cầu hoặc ghi trực tiếp; bank transfer tự động qua SePay |
...
5. **Thanh toán tự động qua SePay** — bank transfer tự xác nhận khi webhook khớp; payment do dealer khai báo chỉ làm giảm công nợ sau khi admin/kế toán xác nhận; admin có thể ghi override.
...
| Payment verification workflow | Đã cập nhật ở mức tài liệu: dealer gửi yêu cầu, admin/kế toán xác nhận | Cần đồng bộ lại code/API/DB để phản ánh đúng `PENDING_VERIFICATION` / `CONFIRMED` / `REJECTED` |

---

*Cập nhật: 2026-03-22 — Điều chỉnh tài liệu payment/debt workflow theo hướng yêu cầu admin verification trước khi trừ công nợ.*
