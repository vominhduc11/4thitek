...
5. Reserve serial: khóa row `Product` và serial liên quan, pick đúng số serial `AVAILABLE → RESERVED` trong cùng transaction; sau đó đồng bộ `product.stock` như giá trị phái sinh
...
> Runtime hiện tại dùng pool serial `AVAILABLE` làm nguồn sự thật để chặn oversell giữa các dealer. `Product.stock` chỉ còn là số liệu đồng bộ/phái sinh từ pool serial khả dụng của SKU.
