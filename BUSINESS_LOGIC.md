...
TĂ i liá»‡u nĂ y lĂ  **canonical business contract** cho há»‡ thá»‘ng 4thitek ká»ƒ tá»« phiĂªn báº£n `2026-03-22`.
Mọi má»¥c **khĂ´ng** gáº¯n `[Pending Decision]` pháº£i Ä‘Æ°á»£c xem lĂ  rule production. Náº¿u code, QA, hoáº·c frontend lá»‡ch vá»›i cĂ¡c rule nĂ y thĂ¬ pháº£i coi Ä‘Ă¢y lĂ  defect hoáº·c cĂ³ quyáº¿t Ä‘á»‹nh thay Ä‘á»•i contract.
...
### 0.4 Pháº¡m vi cĂ²n má»Ÿ / chÆ°a chá»‘t production contract

- Reserved-timeout policy cho Ä‘Æ¡n treo quĂ¡ lĂ¢u
- RMA / reset serial tá»« `DEFECTIVE` hoáº·c `RETURNED`
- Debt payment verification workflow nhiá»u bÆ°á»›c
- SePay partial / over payment
- Support SLA vĂ  support thread nhiá»u lÆ°á»£t
- Refund / reversal trong há»‡ thá»‘ng
...
- `BUSINESS_LOGIC.md`: quy táº¯c nghiá»‡p vá»¥, lifecycle, contract production
- `RUNTIME_BEHAVIOR.md`: runtime behavior, cache, sync, transport, compatibility notes
- `CLIENT_NOTES.md`: ghi chĂº implementation cho Flutter/React/Next.js

> CĂ¡c chi tiáº¿t thuáº§n implementation nhÆ° local persistence, optimistic update, widget lifecycle, hay tĂªn mĂ n hĂ¬nh legacy khĂ´ng nĂªn Ä‘Æ°á»£c dĂ¹ng Ä‘á»ƒ thay Ä‘á»•i business contract. CĂ¡c ghi chĂº Ä‘Ă³ thuá»™c `RUNTIME_BEHAVIOR.md` hoáº·c `CLIENT_NOTES.md`.
...
#### Ma tráº­n `order.status` vĂ  `paymentStatus` `[Implemented]`

`paymentStatus` hiá»‡n táº¡i lĂ  aggregate field á»Ÿ level Order, Ä‘Æ°á»£c suy ra tá»« `paymentMethod`, `paidAmount`, `totalAmount`, vĂ  `order.status`.

| `order.status` | `paymentStatus` há»£p lá»‡ trong runtime hiá»‡n táº¡i | Diá»…n giáº£i |
|---|---|---|
| `PENDING` | `PENDING`, `DEBT_RECORDED`, `PAID` | ChÆ°a duyá»‡t Ä‘Æ¡n; cĂ³ thá»ƒ chÆ°a thanh toĂ¡n, ghi ná»£, hoáº·c Ä‘Ă£ thanh toĂ¡n |
| `CONFIRMED` | `PENDING`, `DEBT_RECORDED`, `PAID` | ÄÆ¡n Ä‘Ã£ duyá»‡t nhÆ°ng chÆ°a giao xong |
| `SHIPPING` | `PENDING`, `DEBT_RECORDED`, `PAID` | Äang giao hĂ ng; khĂ´ng cĂ³ partial-payment status riĂªng trong runtime hiá»‡n táº¡i |
| `COMPLETED` | `PENDING`, `DEBT_RECORDED`, `PAID` | HoĂ n táº¥t giao hĂ ng; serial vĂ o kho dealer |
| `CANCELLED` | `CANCELLED`, `PENDING`, `DEBT_RECORDED`, `PAID` | `CANCELLED` chá»‰ xáº£y ra khi há»§y Ä‘Æ¡n vĂ  `paidAmount = 0`; cĂ¡c combo cĂ²n láº¡i cĂ³ nghÄ©a lĂ  Ä‘Ã£/Ä‘ang cĂ³ nghÄ©a vá»¥ tĂ i chĂ­nh cáº§n xá»­ lĂ½ manual |

**Quy táº¯c khá»‘ng cháº¿:**
- `FAILED` khĂ´ng pháº£i aggregate `order.paymentStatus` trong runtime hiá»‡n táº¡i; Ä‘Ă¢y lĂ  tháº¥t báº¡i cá»§a giao dá»‹ch/attempt riĂªng, khĂ´ng pháº£i trạng thái chuẩn cá»§a Order
- `CANCELLED` khĂ´ng há»£p lá»‡ cho order chÆ°a `CANCELLED`
- CĂ¡c tá»• há»£p `CANCELLED + (PENDING|DEBT_RECORDED|PAID)` khĂ´ng bá»‹ runtime chặn, nhÆ°ng pháº£i Ä‘Æ°á»£c xem lĂ  **manual finance follow-up required**

#### Dealer há»§y Ä‘Æ¡n `[Implemented]`

- Dealer chá»‰ Ä‘Æ°á»£c há»§y `PENDING` hoáº·c `CONFIRMED`
- Dealer **khĂ´ng** Ä‘Æ°á»£c há»§y `SHIPPING`, `COMPLETED`, `CANCELLED`
- Khi há»§y: serial `RESERVED â†’ AVAILABLE`, Ä‘Æ¡n chuyá»ƒn `CANCELLED`, vĂ  backend gá»­i notification cho admin
- Náº¿u há»§y khi `paidAmount = 0` â†’ `paymentStatus = CANCELLED`
- Náº¿u há»§y khi Ä‘Æ¡n Ä‘Ă£ cĂ³ `paidAmount > 0` â†’ runtime **khĂ´ng** tá»± refund, khĂ´ng tá»± reverse debt, khĂ´ng tá»± sinh compensating transaction; `paymentStatus` sáº½ Ä‘Æ°á»£c giá»¯ theo aggregate hiá»‡n hĂ nh vĂ  cáº§n xá»­ lĂ½ manual

> NghÄ©a là dealer cĂ³ thá»ƒ há»§y `CONFIRMED` ká»ƒ cáº£ khi Ä‘Æ¡n Ä‘Ă£ cĂ³ thanh toĂ¡n há»£p lá»‡. Viá»‡c hoĂ n tiá»n / xĂ³a cĂ´ng ná»£ / bĂºt toĂ¡n Ä‘á»‘i á»©ng **chÆ°a** Ä‘Æ°á»£c tá»± Ä‘á»™ng hĂ³a trong runtime hiá»‡n táº¡i.
...
- `AVAILABLE`: tĂ i sáº£n cá»§a NPP, chÆ°a giá»¯ cho Ä‘Æ¡n nĂ o
- `RESERVED`: tĂ i sáº£n cá»§a NPP, Ä‘Ã£ giá»¯ cho má»™t order, chÆ°a thuá»™c dealer
- `ASSIGNED`: Ä‘Ã£ vĂ o kho dealer, thuá»™c dealer
- `WARRANTY`: Ä‘Ã£ bĂ¡n ra vĂ  kĂ­ch hoáº¡t cho khĂ¡ch cuá»‘i
...
- Mapping hiá»‡n táº¡i: `WARRANTY` Ä‘Æ°á»£c render nhÆ° `warranty` trong Dealer App
...
- **Theme:** `light` | `dark` | `system` â€” `system` theo setting thiáº¿t bá»‹.
...
**Tráº¡ng thĂ¡i section:** `[Implemented]` cho cĂ¡c field runtime hiá»‡n hĂ nh; `[Pending Decision]` cho migration legacy chÆ°a xĂ³a háº¿t.

**Canonical fields:**
- `orderCode` lĂ  business identifier chuáº©n cho Ä‘Æ¡n hĂ ng; `id` DB khĂ´ng Ä‘Æ°á»£c dĂ¹ng thay cho `orderCode` á»Ÿ search/display flow user-facing
- `product.stock` lĂ  field **derived/read model**, khĂ´ng pháº£i inventory input canonical
- Pool serial kháº£ dá»¥ng `dealer IS NULL AND order IS NULL AND status = AVAILABLE` lĂ  source of truth Ä‘á»ƒ cháº·n oversell
- Response import serial summary `importedItems`, `skippedItems`, `importedCount`, `skippedCount` lĂ  contract chuáº©n cho cĂ¡c FE
- `paymentStatus` lĂ  aggregate field á»Ÿ level Order; `Payment` record lĂ  source-of-truth cho tá»«ng láº§n ghi nháº­n/giao dá»‹ch

**Legacy / compatibility rules:**
- `id` váº«n cĂ³ thá»ƒ xuáº¥t hiá»‡n trong API Ä‘á»ƒ nĂ´i liĂªn ná»™i bá»™, nhÆ°ng khĂ´ng Ä‘Æ°á»£c xem lĂ  business code thay cho `orderCode`
- CĂ¡c field legacy/chuyá»ƒn tiáº¿p chá»‰ Ä‘Æ°á»£c dĂ¹ng cho read compatibility; má»i write flow má»›i pháº£i bĂ¡m canonical schema hiá»‡n hĂ nh
...
- `400`: request sai, validation fail, business rule fail, payload khĂ´ng há»£p lá»‡
- `401`: chÆ°a xĂ¡c thá»±c, token háº¿t háº¡n, refresh fail, dealer khĂ´ng `ACTIVE`
- `403`: Ä‘Ã£ xĂ¡c thá»±c nhÆ°ng khĂ´ng Ä‘á»§ quyá»n
- `404`: resource khĂ´ng tá»“n táº¡i
- `409`: conflict nhÆ° duplicate payment, transactionCode Ä‘Ã£ tá»“n táº¡i, duplicate mutation window
- `413`: file quĂ¡ dung lÆ°á»£ng
- `429`: vÆ°á»£t rate limit
- `500`: lá»—i há»‡ thá»‘ng khĂ´ng lÆ°á»ng trÆ°á»›c

**Current conventions:**
- KhĂ´ng dĂ¹ng `422` trong runtime hiá»‡n táº¡i
- FE pháº£i bĂ¡m `status code + envelope` thay vĂ¬ suy luáº­n tá»« raw text

---

### 3.24 Audit & Traceability

**Tráº¡ng thĂ¡i section:** `[Implemented]` cho admin mutation audit. `[Pending Decision]` cho retention/access policy Ä‘áº§y Ä‘á»§.

**Admin mutation audit `[Implemented]`:**
- Má»i `POST`, `PUT`, `PATCH`, `DELETE` Ä‘i qua `AdminController` Ä‘Æ°á»£c ghi audit log backend
- Audit payload hiá»‡n táº¡i gá»“m: actor, actorRole, action, requestMethod, requestPath, entityType, entityId, ipAddress, payload
- CĂ¡c field nháº¡y cáº£m chá»©a `password`, `token`, `secret`, `apiKey`, `accessKey`, `checksum` pháº£i bá»‹ redact trÆ°á»›c khi lÆ°u

**Pending decisions:**
- ai Ä‘Æ°á»£c xem audit log
- retention bao lĂ¢u
- cĂ³ cáº§n export/search UI riĂªng hay khĂ´ng

---

### 3.25 Refund & Reversal

**Tráº¡ng thĂ¡i section:** `[Implemented]` cho current limitation. `[Pending Decision]` cho refund/reversal workflow Ä‘áº§y Ä‘á»§.

**Current runtime `[Implemented]`:**
- KhĂ´ng cĂ³ endpoint refund/payment reversal chuáº©n trong há»‡ thá»‘ng
- KhĂ´ng cĂ³ negative payment / compensating transaction flow production
- Há»§y order khĂ´ng tá»± refund, khĂ´ng tá»± reverse debt, khĂ´ng xĂ³a payment Ä‘Ã£ ghi
- Payment record khĂ´ng nĂªn bá»‹ xĂ³a cá»©ng Ä‘á»ƒ biá»ƒu diá»…n hoĂ n tiá»n; Ä‘Ã¢y chÆ°a pháº£i workflow há»— trá»£ trong runtime

**Pending decisions:**
- order `PAID` bá»‹ há»§y thĂ¬ refund trong há»‡ thá»‘ng hay xá»­ lĂ½ ngoĂ i há»‡ thá»‘ng
- payment nháº­p sai sá»­a báº±ng reverse entry, admin adjustment, hay refund workflow riĂªng
- debt reversal khi há»§y order ghi ná»£
...
- Xem `RUNTIME_BEHAVIOR.md` cho optimistic update, sync retry, vĂ  mutation conflict handling.
- Business contract cá»‘t lĂµi váº«n lĂ : cart mutation khĂ´ng Ä‘Æ°á»£c pháº¡ vĂ»¡ inventory rule vĂ  auth rule cá»§a backend.

### 5.5 Giao diá»‡n & CĂ i Ä‘áº·t

- Xem `CLIENT_NOTES.md` cho widget lifecycle, local preference persistence, vĂ  cĂ¡c implementation detail khĂ´ng thuá»™c business contract.
- Business contract cá»‘t lĂµi á»Ÿ section nĂ y chá»‰ gá»“m: user cĂ³ thá»ƒ chá»n theme `light|dark|system` vĂ  ngĂ´n ngá»¯ `vi|en`.
...
| Finance rule cho order `CANCELLED` nhÆ°ng váº«n cĂ²n nghÄ©a vá»¥ thanh toĂ¡n | Ma tráº­n runtime Ä‘Ã£ document á»Ÿ [3.4](#34-Ä‘áº·t-hĂ ng--thanh-toĂ¡n); `CANCELLED + (PENDING|DEBT_RECORDED|PAID)` hiá»‡n Ä‘Æ°á»£c xem lĂ  manual follow-up | Cáº§n chá»‘t refund/reversal/debt cleanup policy |
...
| Support thread nhiá»u lÆ°á»£t | Runtime hiá»‡n táº¡i lĂ  mĂ´ hĂ¬nh má»™t chiá»u + `adminReply` Ä‘Æ¡n | Cáº§n chá»‘t cĂ³ nĂ¢ng cáº¥p sang thread/attachment hay khĂ´ng |
...
### 7.3 Detailed Pending Decisions To Close Before Calling The System Fully Production-Final

#### Reserved timeout `[Pending Decision]`

**Current runtime:** serial `RESERVED` khĂ´ng cĂ³ timeout tá»± Ä‘á»™ng.

**Decision cáº§n chá»‘t:**
- cĂ³ auto-cancel Ä‘Æ¡n `PENDING` quĂ¡ X giá» hay khĂ´ng
- khi auto-cancel cĂ³ release toàn bá»™ serial `RESERVED â†’ AVAILABLE` hay khĂ´ng
- náº¿u order Ä‘Ã£ cĂ³ nghÄ©a vá»¥ thanh toĂ¡n há»£p lá»‡ thĂ¬ auto-cancel hay chuyá»ƒn queue manual
- ai nháº­n notification (dealer, admin, cáº£ hai)

**Recommended default náº¿u chÆ°a cĂ³ quyáº¿t Ä‘á»‹nh khĂ¡c:** auto-cancel order `PENDING` quĂ¡ timeout, release serial, nhÆ°ng chuyá»ƒn manual review náº¿u order Ä‘Ã£ cĂ³ payment há»£p lá»‡.

#### RMA / serial reset cho `DEFECTIVE` vĂ  `RETURNED` `[Pending Decision]`

**Current runtime:** `DEFECTIVE` vĂ  `RETURNED` chÆ°a cĂ³ state machine Ä‘áº§y Ä‘á»§ Ä‘á»ƒ quay láº¡i inventory há»£p phĂ¡p.

**Decision cáº§n chá»‘t:**
- ai Ä‘Æ°á»£c chuyá»ƒn `DEFECTIVE` / `RETURNED` sang cáº£nh xá»­ lĂ½ tiáº¿p theo
- cĂ³ cáº§n image/proof/QC note hay khĂ´ng
- ownership thay Ä‘á»•i á»Ÿ bá»›c nĂ o
- warranty cĂ³ bá»‹ vĂ´ hiá»‡u hoáº·c treo khi serial Ä‘i vĂ o RMA hay khĂ´ng

**Recommended shape:** tĂ¡ch riĂªng luá»“ng QC/inspection cho `DEFECTIVE` vĂ  `RETURNED`, khĂ´ng cho reset tháº³ng vá» `AVAILABLE` nĂ¿n chÆ°a qua bá»›c trung gian.

#### Debt payment verification `[Pending Decision]`

**Current runtime:** dealer ghi payment cho Ä‘Æ¡n cĂ²n ná»£ vĂ  payment cĂ³ hiá»‡u lá»±c ngay.

**Decision cáº§n chá»‘t:**
- cĂ³ Ä‘Æ°a payment dealer vĂ o `PENDING_VERIFICATION` hay khĂ´ng
- admin xĂ¡c nháº­n/tá»« chá»‘i theo SLA nĂ o
- khi tá»« chá»‘i thĂ¬ proof cÅ© vĂ  payment record cá»§ xá»­ lĂ½ ra sao

**Nếu giá»¯ flow hiá»‡n táº¡i:** file nĂ y xem Ä‘Ă¢y lĂ  business decision chá»§ Ä‘á»™ng chấp nhận rủi ro khai báo thanh toán từ dealer, không phải bug.

#### SePay partial / over payment `[Pending Decision]`

**Current runtime:** chá»‰ chấp nhận giao dịch khá»›p **Ä‘Ăºng báº±ng** `outstandingAmount`.

**Decision cáº§n chá»‘t:**
- thiáº¿u tiá»n: bá» qua hay ghi `partial`
- nhiá»u láº§n chuyá»ƒn: cá»™ng dá»“n hay khĂ´ng
- dÆ° tiá»n: táº¡o credit balance hay tá»« chá»‘i

**Nếu giá»¯ hÆ°á»›ng Ä‘Æ¡n giáº£n:** pháº£i xem `thiếu`, `nhiều lần`, `dư tiền` Ä‘á»u lĂ  unsupported by policy.

---

*Cáº­p nháº­t: 2026-03-23 â€” Siáº¿t láº¡i `BUSINESS_LOGIC.md` thĂ nh canonical contract, bá»• sung matrix order/payment, error contract, audit/traceability, refund/reversal notes, public dealer visibility, vĂ  tĂ¡ch implementation detail sang `RUNTIME_BEHAVIOR.md` / `CLIENT_NOTES.md`.*
