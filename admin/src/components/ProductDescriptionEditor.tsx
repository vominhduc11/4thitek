...
export function ProductDescriptionEditor({ value, onChange }: ProductDescriptionEditorProps) {
  ...
  const renderPreview = (item: ProductOverviewItem, index: number) => {
    switch (item.type) {
      ...
      case 'description':
        return <div className="description-content text-muted-foreground text-sm" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(item.text || 'Mô tả trống') }} />;
      case 'image':
        return (
      ...
      case 'images':
        return (
          <div className="text-sm text-primary">
            <span className="font-medium">Nhiều hình ảnh sản phẩm ({item.images?.length || 0})</span>
            ...
            <CardContent>
              {editingIndex === index ? (
                <div className="space-y-3">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(type: 'title' | 'description' | 'image' | 'images' | 'video') => {
                        const updates: Partial<ProductOverviewItem> = { type };
                  ...
                  {item.type === 'images' && (
                    <div>
                      <label className="text-sm font-medium">Chọn nhiều hình ảnh sản phẩm</label>
                      <MultipleImageUpload
                        value={item.images || []}
                        onChange={(images) => updateItem(index, { images })}
                        placeholder="Chọn hoặc kéo thả nhiều hình ảnh sản phẩm"
                        maxSizeInMB={10}
                        maxImages={10}
                        folder="products"
                      />
                  ...
                  {item.type === 'video' && (
                    <div className="space-y-3">
                      ...
                      {item.videoUrl && item.videoUrl.trim() !== '' && !getYouTubeVideoId(item.videoUrl) && (
                        <div className="p-3 bg-red-50 dark:bg-red-950/20 border border-red-300 dark:border-red-800/50 rounded-lg">
                      ...
                      {item.videoUrl && getYouTubeVideoId(item.videoUrl) && (
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="text-lg">✅</span>
                            <label className="text-sm font-medium text-green-700 dark:text-green-300">URL hợp lệ - Preview:</label>
                          </div>
