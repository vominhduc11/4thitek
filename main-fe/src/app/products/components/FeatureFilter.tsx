    ...
    return (
        ...
        >
            <h3 className="text-lg 2xl:text-xl 3xl:text-2xl font-semibold text-white mb-4 2xl:mb-6 3xl:mb-8 flex items-center gap-2 2xl:gap-3 3xl:gap-4">
                ...
                {t('products.filters.features')}
            </h3>
            <div className="grid grid-cols-1 gap-2 2xl:gap-3 3xl:gap-4">
                {features.map((feature) => (
                    <motion.button
                        key={feature}
                        onClick={() => onToggleFeature(feature)}
                        className={`text-left px-4 py-2 2xl:px-6 2xl:py-3 3xl:px-8 3xl:py-4 rounded-lg 2xl:rounded-xl 3xl:rounded-2xl transition-all duration-300 ${
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <span className="text-sm 2xl:text-base 3xl:text-lg font-medium">{feature}</span>
                            {selectedFeatures.includes(feature) && (
                                <motion.div
                                    className="w-1.5 h-1.5 2xl:w-2 2xl:h-2 3xl:w-2.5 3xl:h-2.5 bg-[#4FC8FF] rounded-full"
                                    initial={{ scale: 0 }}
                                    animate={{ scale: 1 }}
