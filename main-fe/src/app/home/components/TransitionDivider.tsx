...
export default function TransitionDivider({
    ...
    if (type === 'wave') {
        return (
            <div className={`relative w-full ${heightVariants[height]} overflow-hidden`} aria-hidden="true">
                <div
                ...
                <svg className="absolute bottom-0 w-full h-full" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <motion.path
                        ...
                        fill={toColor}
                        initial={enableDecorativeAnimations ? { pathLength: 0 } : { pathLength: 1 }}
                        animate={{ pathLength: 1 }}
                        transition={{ duration: 2, ease: 'easeInOut' }}
                    />
                </svg>

                {/* Animated particles - only when infinite animations are enabled */}
    ...
    if (type === 'diagonal') {
        return (
            <div className={`relative w-full ${heightVariants[height]} overflow-hidden`} aria-hidden="true">
                ...
                {/* Diagonal lines - only when infinite animations are enabled */}
                {enableInfiniteAnimations &&
                    [...Array(6)].map((_, i) => (
                        <motion.div
                            key={i}
                            className="absolute h-full w-px bg-[#4FC8FF]/10"
                            style={{ left: `${15 + i * 15}%`, transform: 'rotate(15deg)', transformOrigin: 'center' }}
                            initial={{ scaleY: 0, opacity: 0 }}
                            animate={{ scaleY: [0, 1, 0.8], opacity: [0, 0.5, 0.2] }}
    ...
    return (
        <div className={`relative w-full ${heightVariants[height]} overflow-hidden`} aria-hidden="true">
            ...
            {/* Floating gradient overlay - only when infinite animations are enabled */}
            {enableInfiniteAnimations && (
                <motion.div
            ...
            <div className="absolute top-1/2 left-0 right-0 transform -translate-y-1/2">
                <motion.div
                    ...
                    animate={
                        enableInfiniteAnimations
                            ? { opacity: [0.3, 0.8, 0.3], scaleX: [0.8, 1.2, 0.8] }
