...
import { BeatLoader, ClipLoader, DotLoader, HashLoader, PulseLoader, RingLoader, SyncLoader } from 'react-spinners';
...
export default function Spinner({
    ...
    const content = (
        <div className={`flex flex-col items-center justify-center gap-4 ${className}`}>
            {renderSpinner()}
            {message && <p className="text-sm text-[var(--text-secondary)] sm:text-base animate-pulse">{message}</p>}
        </div>
    );

    if (fullScreen) {
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(6,17,27,0.82)] backdrop-blur-sm">
                {content}
            </div>
        );
    }
