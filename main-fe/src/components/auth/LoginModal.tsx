...
export default function LoginModal({ isOpen, onClose, onSuccess }: LoginModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!response.ok || !payload?.success || !payload.data?.accessToken || !payload.data.user) {
                ...
                login({
                    ...
                    accountId: payload.data.user.id,
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    {errorKey && (
                        <div className="bg-red-900 bg-opacity-20 border border-red-500 rounded-lg p-3 flex items-start space-x-2">
                            ...
                            <p className="text-red-300 text-sm">{t(errorKey)}</p>
                        </div>
                    )}

                    <div>
                        <label className="block text-white font-medium mb-2">{t('auth.login.emailLabel')}</label>
                        <div className="relative">
                    ...
                    <div>
                        <label className="block text-white font-medium mb-2">{t('auth.login.passwordLabel')}</label>
                        <div className="relative">
                        ...
                        <div className="text-right mt-1">
                            <button type="button" onClick={handleForgotPassword} className="text-[#00d4ff] text-sm hover:underline">
